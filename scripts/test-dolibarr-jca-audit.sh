...
echo "2) Dolibarr thirdparties (may return 404 if empty)"
...
echo "4) Submit async tariff change"
...
echo "5) Tariff request status (first check)"
curl -sS "http://localhost:8180/api/v1/subscribers/1/tariff-change-requests/$TARIFF_REQUEST_ID" \
  -H "Authorization: Bearer $TOKEN"
echo
sleep 2
echo "5.1) Tariff request status (second check)"
curl -sS "http://localhost:8180/api/v1/subscribers/1/tariff-change-requests/$TARIFF_REQUEST_ID" \
  -H "Authorization: Bearer $TOKEN"
echo
echo

echo "6) Submit async feature disable"
...
echo "7) Feature disable status (first check)"
curl -sS "http://localhost:8180/api/v1/subscribers/1/feature-disable-requests/$FEATURE_REQUEST_ID" \
  -H "Authorization: Bearer $TOKEN"
echo
sleep 2
echo "7.1) Feature disable status (second check)"
curl -sS "http://localhost:8180/api/v1/subscribers/1/feature-disable-requests/$FEATURE_REQUEST_ID" \
  -H "Authorization: Bearer $TOKEN"
echo
echo
